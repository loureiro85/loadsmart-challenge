...
left join (select date as date_p, year_month as year_month_p, weekday_name as weekday_name_p from {{ ref('dim_calendar') }})
on date(pickup_date) = date_p

left join (select date as date_d, year_month as year_month_d, weekday_name as weekday_name_d from {{ ref('dim_calendar') }})
